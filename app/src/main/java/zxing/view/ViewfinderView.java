...
public final class ViewfinderView extends View {
	...
	private int ScanCode = 0;//1需要继续扫描，0不需要扫描
	private String result="";
	private int timeLeftLong  =2*1000;
		...
		if (resultBitmap != null) {
			...
			if ((CaptureActivity.currentState != null)
				...
				if(ScanCode ==1)
				{
					if(timeLeftLong>0)
					{
						if(timeLeftLong%1000==0)
						{
							timeLeft = timeLeftLong/1000;
						}
						canvas.drawText(result+","+timeLeft+"s后继续扫描", frame.centerX(), (float) (frame.bottom + (float) 40 * density), paint);
						timeLeftLong =timeLeftLong-10 ;
					}else{
						if(!isBeginScan)
						{
							isBeginScan = true;
							ScanCode = 0;
//							LogUtil.d("dfy","倒计时结束，开始下次扫描");
							mHandler.sendEmptyMessageDelayed(R.id.restart_preview,timeLeftLong);

						}
					}

				}else{
					canvas.drawText(getResources().getString(R.string.scan_onecode), frame.centerX(), (float) (frame.bottom + (float) 40 * density), paint);
				}
	 ...
	 * 提示扫描成功，继续扫描
	 */
	public void drawViewfinderGoon(String text) {
		Bitmap resultBitmap = this.resultBitmap;
		this.resultBitmap = null;
		if (resultBitmap != null) {
			resultBitmap.recycle();
